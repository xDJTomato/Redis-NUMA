...
#
# 用法: ./test_composite_lru_cxl.sh [redis端口]
# 示例: ./test_composite_lru_cxl.sh 6379
...
LOG_FILE="/tmp/composite_lru_cxl_test.log"
RESULT_FILE="/tmp/composite_lru_cxl_result.txt"
...
log_cxl() {
    echo -e "${CYAN}[CXL]${NC} $1" | tee -a $LOG_FILE
}
...
# 测试5: CXL超高压测试（6层内存架构，超过8GB）
test_ultra_high_pressure() {
    log_info "测试5: CXL超高压测试（6层内存架构，目标超过8GB）..."
    
    local target_memory_gb=8
    local REDIS_BENCH="./src/redis-benchmark -p $REDIS_PORT -n 100000 --csv"
    
    # 阶段1: 使用redis-benchmark快速创建大量数据
    log_info "阶段1: 使用redis-benchmark快速创建CXL分层数据..."
    log_cxl "目标内存: ${target_memory_gb}GB+"
    
    local start_time=$(date +%s)
    
    # 使用redis-benchmark创建不同大小的数据
    # -d 参数指定数据大小（字节）
    
    log_info "创建DRAM层数据（1KB，10万个key）..."
    ./src/redis-benchmark -p $REDIS_PORT -n 100000 -r 100000 -d 1024 -t set --csv > /dev/null 2>&1 &
    local pid1=$!
    
    log_info "创建CXL近端数据（64KB，2万个key）..."
    ./src/redis-benchmark -p $REDIS_PORT -n 20000 -r 20000 -d 65536 -t set --csv > /dev/null 2>&1 &
    local pid2=$!
    
    log_info "创建CXL远端数据（512KB，5000个key）..."
    ./src/redis-benchmark -p $REDIS_PORT -n 5000 -r 5000 -d 524288 -t set --csv > /dev/null 2>&1 &
    local pid3=$!
    
    log_info "创建CXL扩展数据（2MB，2000个key）..."
    ./src/redis-benchmark -p $REDIS_PORT -n 2000 -r 2000 -d 2097152 -t set --csv > /dev/null 2>&1 &
    local pid4=$!
    
    log_info "创建巨型对象（8MB，500个key）..."
    ./src/redis-benchmark -p $REDIS_PORT -n 500 -r 500 -d 8388608 -t set --csv > /dev/null 2>&1 &
    local pid5=$!
    ...
    if [ "$current_mem_mb" -lt "$target_mb" ]; then
        ...
        local extra_count=$((extra_needed_mb / 8))  # 每个8MB对象约占用8MB
        
        log_info "额外创建 $extra_count 个8MB对象..."
        ./src/redis-benchmark -p $REDIS_PORT -n $extra_count -r $extra_count -d 8388608 -t set --csv > /dev/null 2>&1
    ...
    ./src/redis-benchmark -p $REDIS_PORT -n 1000000 -t get -k uhp:dram:1:__rand_int__ --csv > /dev/null 2>&1
    
    local hot_time=$(($(date +%s) - hot_start))
    log_success "热点访问完成，耗时 ${hot_time}秒"
    
    # 阶段3: 批量迁移测试
    log_info "阶段3: 批量迁移测试（使用脚本迁移）..."
    local migrate_start=$(date +%s)
    local migrate_total=0
    local migrate_ok=0
    
    # 迁移各层的热点key
    for layer in dram near far giant; do
        local limit=100
        [ "$layer" = "giant" ] && limit=50
        
        for i in $(seq 1 $limit); do
            local result=$($REDIS_CLI NUMAMIGRATE KEY "uhp:${layer}:1:${i}" 0 2>/dev/null || echo "ERR")
            migrate_total=$((migrate_total + 1))
            [ "$result" = "OK" ] && migrate_ok=$((migrate_ok + 1))
        done
        log_cxl "$layer 层迁移完成: $limit 个"
    done
    ...
    ./src/redis-benchmark -p $REDIS_PORT -n 1000000 -t set,get -d 1024 -k uhp:dram:2:__rand_int__ --csv > /dev/null 2>&1
    ...
    for layer in dram near far giant; do
        ...
        [ "$layer" = "giant" ] && check_count=20
        
        for i in $(seq 1 $check_count); do
            local val=$($REDIS_CLI GET "uhp:${layer}:1:${i}" 2>/dev/null)
            if [ -n "$val" ]; then
    ...
    # 显示统计
    log_info "最终NUMA迁移统计:"
    $REDIS_CLI NUMAMIGRATE STATS 2>/dev/null | sed 's/^/  /' || true
    
    log_info "CXL各层对象统计:"
    for layer in dram near far ext giant; do
        local cnt=$($REDIS_CLI KEYS "uhp:${layer}:*" 2>/dev/null | wc -l)
        log_cxl "$layer 层: $cnt 个"
    done
    
    local final_mem_human=$($REDIS_CLI INFO memory | grep used_memory_human | cut -d: -f2 | tr -d '\r')
    log_info "最终内存使用: $final_mem_human"
    
    # 阶段6: 清理
    log_info "阶段6: 清理CXL超高压测试数据..."
    local cleanup_start=$(date +%s)
    
    $REDIS_CLI FLUSHDB > /dev/null 2>&1
    
    local cleanup_time=$(($(date +%s) - cleanup_start))
    log_success "CXL超高压测试数据清理完成，耗时 ${cleanup_time}秒"
}
...
generate_detailed_report() {
    ...
    echo "" > $RESULT_FILE
    echo "========================================" >> $RESULT_FILE
    echo "Composite LRU策略 - CXL超高压测试报告" >> $RESULT_FILE
    echo "========================================" >> $RESULT_FILE
    echo "测试时间: $(date)" >> $RESULT_FILE
    echo "" >> $RESULT_FILE
    
    echo "【系统环境】" >> $RESULT_FILE
    echo "内核版本: $(uname -r)" >> $RESULT_FILE
    echo "CPU: $(grep 'model name' /proc/cpuinfo | head -1 | cut -d: -f2 | xargs)" >> $RESULT_FILE
    echo "内存: $(free -h | grep Mem | awk '{print $2}')" >> $RESULT_FILE
    echo "" >> $RESULT_FILE
    
    echo "【CXL设备】" >> $RESULT_FILE
    if [ -d "/sys/bus/cxl" ]; then
        echo "CXL总线: 已启用" >> $RESULT_FILE
        echo "CXL设备数: $(ls /sys/bus/cxl/devices/ 2>/dev/null | wc -l)" >> $RESULT_FILE
    else
        echo "CXL总线: 未启用" >> $RESULT_FILE
    fi
    echo "" >> $RESULT_FILE
    
    echo "【NUMA信息】" >> $RESULT_FILE
    echo "NUMA节点数: $(ls -d /sys/devices/system/node/node* 2>/dev/null | wc -l)" >> $RESULT_FILE
    echo "" >> $RESULT_FILE
    
    echo "【Redis信息】" >> $RESULT_FILE
    echo "Redis版本: $($REDIS_CLI INFO server | grep redis_version | cut -d: -f2 | tr -d '\r')" >> $RESULT_FILE
    echo "内存使用: $($REDIS_CLI INFO memory | grep used_memory_human | cut -d: -f2 | tr -d '\r')" >> $RESULT_FILE
    echo "Key数量: $($REDIS_CLI DBSIZE)" >> $RESULT_FILE
    echo "" >> $RESULT_FILE
    
    echo "【CXL 6层对象统计】" >> $RESULT_FILE
    for layer in dram near far ext giant monster; do
        local cnt=$($REDIS_CLI KEYS "uhp:${layer}:*" 2>/dev/null | wc -l)
        echo "  $layer 层: $cnt 个" >> $RESULT_FILE
    done
    echo "" >> $RESULT_FILE
    
    echo "【迁移统计】" >> $RESULT_FILE
    $REDIS_CLI NUMAMIGRATE STATS 2>/dev/null >> $RESULT_FILE || echo "无法获取迁移统计" >> $RESULT_FILE
    echo "" >> $RESULT_FILE
    
    echo "========================================" >> $RESULT_FILE
    echo "测试完成" >> $RESULT_FILE
    echo "详细日志: $LOG_FILE" >> $RESULT_FILE
    echo "========================================" >> $RESULT_FILE
    
    cat $RESULT_FILE | tee -a $LOG_FILE
    
    log_info "报告已保存到: $RESULT_FILE"
}

# 主函数
main() {
    echo "========================================" | tee $LOG_FILE
    echo "Composite LRU策略CXL超高压测试脚本" | tee -a $LOG_FILE
    echo "========================================" | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE

...
| **策略槎位（strategy slot，已退役）** | `numa_strategy_slots`（已随 [ADR-08](09-architecture-decisions.md) 删除）曾提供的 16 个可插拔"迁移策略"挂载点，每个槎位通过 vtable 实现统一的执行接口。三个迁移策略现已统一收敛为 NUMAflow 的原子操作 DAG 预设，不再有槎位/vtable 这层概念。 |
| **Composite LRU** | 迁移策略之一：双通道设计——热候选环形缓冲区（快路径）+ 渐进式字典扫描（慢路径），基于阶梯式惰性衰减的热度值决定迁移。曾是内核槎位 1 的默认原生实现，现已随 ADR-08 统一收敛为 NUMAflow 的原子操作 DAG 预设，通过 `NUMA FLOW DEFAULT composite_lru` 使用。 |
| **TinyLFU** | 频率驱动的迁移策略之一：用 Count-Min Sketch + Doorkeeper 布隆过滤器以固定内存（~40KB）近似统计访问频率。曾是内核槎位 2 的原生实现（默认关闭、实践中从未被真正启用过），现已随 ADR-08 统一收敛为 NUMAflow 的原子操作 DAG 预设，通过 `NUMA FLOW DEFAULT tinylfu` 使用。 |
...
| **AE time event（AE 定时事件，已退役）** | Redis 自身事件循环（`ae.c`）提供的定时任务机制。曾经 `numa_strategy_slots` 支持把某个策略槎位的调度从 `serverCron`（每秒一次、与其它槎位共享周期）切换为独立的 AE time event。这套逐槎位调度切换已随槎位框架一起退役（[ADR-08](09-architecture-decisions.md)）——现在唯一剩下的调度路径是 `numa_flow_cron()` 按 `numa-flow-interval-sec` 判断是否该跑，单线程 `serverCron` 驱动，没有 AE 变体。 |

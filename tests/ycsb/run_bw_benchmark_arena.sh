#!/usr/bin/env bash
# ============================================================================
# NUMA 带宽饱和基准测试脚本 (Arena分配器版本)
#
# 与 run_bw_benchmark.sh 相同，但 Redis 使用 --numa-allocator-type arena
# ============================================================================
...
#   --output-dir DIR     输出目录 (默认: results/bw_arena_<timestamp>)
...
start_redis() {
    ...
    log "启动 Redis (port=$REDIS_PORT, maxmemory=$MAX_MEMORY, allocator=arena)..."
...
main() {
    ...
    if [[ -z "$OUTPUT_DIR" ]]; then
        OUTPUT_DIR="$SCRIPT_DIR/results/bw_arena_$(date +%Y%m%d_%H%M%S)"
    fi
    mkdir -p "$OUTPUT_DIR"

...
manual_migrate() {
    ...
    # 迁移热点 key 到节点 1
    echo -e "${YELLOW}迁移热点 key 到节点 1...${NC}"
    for i in $(seq 1 5); do
        key="${TEST_KEY_PREFIX}_${i}"
        result=$(./src/redis-cli -p $REDIS_PORT NUMAMIGRATE KEY "$key" 1 2>/dev/null)
        if [ "$result" = "OK" ]; then
            echo -e "  ${GREEN}✓${NC} $key 迁移到节点 1 成功"
        else
    ...
    # 查询迁移后的 Key 信息
    echo -e "\n${YELLOW}查询迁移后的 Key 信息:${NC}"
    for i in $(seq 1 3); do
        key="${TEST_KEY_PREFIX}_${i}"
        echo -e "  $key:"
        ./src/redis-cli -p $REDIS_PORT NUMAMIGRATE INFO "$key" 2>/dev/null | grep -E "(current_node|hotness)" | sed 's/^/    /'
    done
}

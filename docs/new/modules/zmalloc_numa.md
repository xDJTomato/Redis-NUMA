...
```text
               ...
               │              zmalloc 分配的总内存             │
               ├──────────────────────┬───────────────────────┤
               │  numa_alloc_prefix_t │     用户可用数据区     │
               │       (16 Bytes)     │  (robj / SDS / dict)  │
               └──────────────────────┴───────────────────────┘
               ▲                      ▲
               │                      │
            raw_ptr                user_ptr (返回给 Redis 上层)
               │                      │
               └─ ptr - PREFIX_SIZE ──┘ (numa_get_prefix: O(1) 指针倒退运算)

 [PREFIX 内部 16 字节字段排布]
 ┌──────────────┬───────────┬─────────┬─────────┬──────────────┬─────────────┬──────────┬──────────┐
 │ size (8B)    │from_slab  │node_id  │hotness  │access_count  │last_access  │migrated  │reserved  │
 │ 实际分配大小 │(1B Slab标)│(1B 节点)│(1B 热度)│(1B 频次计数) │(2B LRU时钟) │(1B 亲和) │(1B 预留) │
 └──────────────┴───────────┴─────────┴─────────┴──────────────┴─────────────┴──────────┴──────────┘
```
...
```text
               zmalloc(size) 请求到达
                         │
                         ▼
             numa_config_get_best_node(size) ──► 选定 NUMA 目标节点
                         │
        ┌────────────────┼────────────────┐
        ▼ (≤ 64KB)       ▼ (64KB ~ 2MB)   ▼ (> 2MB)
┌───────────────┐ ┌───────────────┐ ┌───────────────────┐
│  tcache (TLS) │ │Direct Cache(TLS)│ numa_alloc_onnode │
│   未命中 ──┐   │ │   未命中 ──┐   │ (mmap + mbind 系统调用)
└────────────┼──┘ └────────────┼──┘ └─────────┬─────────┘
             ▼                 ▼              │
    numa_slab_alloc    numa_alloc_onnode      │
    (两级 Slab 分配器)   (Direct 分配)         │
             │                 │              │
             └─────────────────┴──────────────┘
                               │
                               ▼

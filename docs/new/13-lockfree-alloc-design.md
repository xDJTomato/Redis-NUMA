...
zmalloc() → numa_alloc_with_size() → numa_config_get_best_node()
    → select_best_node() → [选择目标 NUMA 节点]
    → numa_pool_alloc() / numa_slab_alloc() → [实际分配]
```
...
| 层级 | 锁 | 作用 |
|------|-----|------|
| 节点选择 | `g_config_mutex`（现已消除） | 保护分配目标节点决策 |
| Pool 分配 | `pool->lock`（每 size-class 一个） | 保护 chunk bump-pointer 和 free_list |

两个锁之间无依赖关系。`select_best_node()` 只决定目标节点号，实际的 chunk 内存分配由 `numa_pool_alloc()` 内部的 `pthread_mutex_lock(&pool->lock)` 保护。

Pool 锁的进一步优化（per-thread cache / lock-free bump allocator）属于后续 P1 修复范畴。

...
SIZE_LIST="32,64,128,256,512,1024,2048,4096,8192,16384,32768,65536,131072,262144,524288,1048576"
...
run_variant() {
    ...
    local remote_out="${REMOTE_YCSB}/results/size_sweep_${variant}_${TIMESTAMP}"
    _ssh "mkdir -p ${remote_out}"
    log_step "运行 ${variant} size sweep"
    ...
    for raw_size in "${sizes[@]}"; do
        ...
        case_dir="results/size_sweep_${variant}_${TIMESTAMP}/size_${size}"
        extra_args=""
        if [[ "$variant" == "numa" ]]; then
            extra_args="--numa-strategy ${NUMA_STRATEGY}"
        fi
        log "${variant}: size=${label}, records=${records}, ops=${ops}"
        _ssh "cd ${REMOTE_YCSB} && ${root_arg} bash run_progressive_hotspot.sh --variant ${variant} --port ${port} --output-dir ${case_dir} --records ${records} --fieldlength ${size} --threads ${THREADS} --ops ${ops} --read-only --maxmem ${maxmem} ${extra_args}" || {
            log_warn "${variant} size=${label} 返回非零，继续"
        }
    done
...
fetch_and_plot() {
    ...
    mkdir -p "$fetch_dir/numa" "$fetch_dir/vanilla"
    _scp -r "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_YCSB}/results/size_sweep_numa_${TIMESTAMP}/." "$fetch_dir/numa/"
    _scp -r "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_YCSB}/results/size_sweep_vanilla_${TIMESTAMP}/." "$fetch_dir/vanilla/"

    local numa_csv="$fetch_dir/numa/size_sweep_summary.csv"
    local vanilla_csv="$fetch_dir/vanilla/size_sweep_summary.csv"
    local output="$fetch_dir/size_sweep_compare.png"
    if python3 -c "import matplotlib" 2>/dev/null; then
        python3 "${SCRIPT_DIR}/scripts/visualize_size_sweep.py" \
            --input "$numa_csv" \
            --label "Redis-NUMA interleaved" \
            --compare-input "$vanilla_csv" \
            --compare-label "Vanilla Redis" \
            --output "$output" \
            --title "YCSB Hotspot Read Size Sweep: 32 threads" || log_warn "本地绘图失败"
    else
...
generate_summary() {
    ...
    local summary="$dir/SUMMARY.txt"
    {
        ...
        echo "负载: up to ${RECORD_COUNT} keys, 32 threads, read-only hotspot"
        echo "Key 数量: <16KiB 100000 keys, 16KiB-256KiB 20000 keys, 512KiB 24576 keys, 1MiB 12288 keys"
        echo "数据规模: 512KiB/1MiB 均约 12GiB 用户 value 数据"
        echo "操作数: <16KiB 50000 ops, >=16KiB 10000 ops"
...
main() {
    ...
    sync_remote
    run_variant numa "$TEST_PORT" "$MAXMEM_NUMA"
    run_variant vanilla "$((TEST_PORT+1))" "$MAXMEM_VANILLA"
    fetch_and_plot

...
│  ┌─────────────┐  ┌─────────────────────────────────────────┐  │
│  │ zmalloc.c   │  │ numa_pool.c（含Pool+Slab双路径实现）     │  │
│  │ (NUMA适配)  │  │ Pool路径：256KB/512KB/1MB动态chunk       │  │
│  │             │  │ >128B对象走Pool路径                      │  │
│  │ PREFIX机制   │  │ Slab路径：16KB slab，bitmap O(1)管理    │  │
│  │(16字节元数据)│  │ ≤128B对象走Slab路径                     │  │
│  └──────┬──────┘  └──────────────────────┬──────────────────┘  │
└─────────┼────────────────────────────────┼─────────────────────┘
          │                                │
          ▼                                ▼
...
| 模块 | 文件 | 功能描述 | 状态 |
|------|------|----------|------|
| **NUMA内存池** | `numa_pool.h/c` | 节点粒度的内存池管理，256KB/512KB/1MB动态chunk，16级大小分类 | 已完成 v3.2-P2 |
